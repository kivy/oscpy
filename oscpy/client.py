...
def send_message(
    ...
    sock.sendto(
        format_message(osc_address, values, encoding=encoding),
        address
    )
    if safer:
        sleep(10e-9)
...
def send_bundle(
    ...
    if not sock:
        sock = SOCK
    sock.sendto(
        format_bundle(messages, timetag=timetag, encoding=encoding),
        (ip_address, port),
    )
    if safer:
        sleep(10e-9)
...
class OSCClient(object):
    ...
    '''
    def __init__(self, address, port, sock=None, encoding=''):
