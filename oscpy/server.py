...
class OSCThreadServer(object):
    ...
    def answer(
        self, address=None, values=None, bundle=None, timetag=None, safer=False, port=None
    ):
        ...
        ip_address, responseport = frame.f_locals.get('sender')
        if port != None:
            responseport = port
        ...
        if bundle:
            self.send_bundle(
                bundle, ip_address, responseport, timetag=timetag, sock=sock,
                safer=safer
            )
        else:
            self.send_message(address, values, ip_address, responseport, sock=sock)
